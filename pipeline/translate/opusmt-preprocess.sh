#!/bin/bash
##
# Applies OPUS-MT preprocessing to corpus
#

set -x
set -euo pipefail

# add spm_encode to path
export PATH=$(realpath 3rd_party/marian-dev/build/):$PATH

source_file=$1
opusmt_model=$2
source_lang=$3
spm_name=$4
model_dir=$(dirname $2)
model_index=$5

#target_lang_token needs to be provided for multilingual models
#first check whether model is multilingual
if grep -q ">>id<<" "${model_dir}/README.md"; then
    target_lang_token=$6
    if [ -n "${target_lang_token}" ]; then
        #add space after lang token
        target_lang_token=">>${target_lang_token}<< "
    else
        echo "target multilingual model specified, but no target lang token provided"
        exit 1
    fi
else
    target_lang_token=""
fi


if [ "${source_file##*.}" == "gz" ]; then
    echo "source file is gzipped"
    zcat $1 | pipeline/translate/preprocess.sh $3 "${model_dir}/${spm_name}" | sed -e "s/^/${target_lang_token}/" | gzip > ${source_file%%.gz}.${model_index}.opusmt.gz
else
    echo "source file is not gzipped"
    pipeline/translate/preprocess.sh $3 "${model_dir}/${spm_name}" < $1 | sed -e "s/^/${target_lang_token}/" > $1.${model_index}.opusmt
fi

 
