...
rule data_mono_src:
    ...
    shell: '''bash pipeline/data/download-mono.sh \
                "{src}" "{mono_max_sent_src}" "{original}/mono" "{cache_dir}" {mono_src_datasets} 2>&1 | tee {log}'''
...
if mono_trg_datasets:
    rule data_mono_trg:
        ...
        shell: '''bash pipeline/data/download-mono.sh \
                  "{trg}" "{mono_max_sent_trg}" "{original}/mono" "{cache_dir}" {mono_trg_datasets} 2>&1 | tee {log}'''
...
rule clean_mono_src:
    ...
    shell: '''CLEAN_TOOLS=pipeline/clean/tools \
                bash pipeline/clean/clean-mono.sh "{src}" "{original}/mono" "{clean}/mono" 2>&1 | tee {log}'''
...
rule train_vocab:
    ...
    shell: '''MARIAN={marian_dir} \
              bash pipeline/train/spm-vocab.sh "{input.corpus_src}" "{input.corpus_trg}" "{output}" 2>&1 | tee {log}'''
...
rule teacher:
    ...
    shell: '''SRC={src} TRG={trg} MARIAN={marian_dir} GPUS="{gpus}" WORKSPACE={workspace} \
                ...
                "{output.dir}" "{params.prefix_train}" "{params.prefix_test}" "{input.vocab}" 2>&1 | tee {log}'''
...
rule split_corpus:
    ...
    shell: '''bash pipeline/translate/split-corpus.sh \
                {input.corpus_src} {input.corpus_trg} {translated}/corpus {partitions} 2>&1 | tee {log}'''
...
rule translate_corpus:
    ...
    shell: '''SRC={src} TRG={trg} MARIAN={marian_dir} GPUS="{gpus}" WORKSPACE={workspace} \
                ...
                "{input.files}" "{input.teacher_models}" "{input.vocab}" "{translated}/corpus" 2>&1 | tee {log}'''
...
rule extract_best:
    ...
    shell: '''bash pipeline/translate/extract-best.sh {translated}/corpus {input}'''
...
rule collect_corpus:
    ...
    shell: '''bash pipeline/translate/collect.sh {translated}/corpus {output} {params.src_corpus}'''
