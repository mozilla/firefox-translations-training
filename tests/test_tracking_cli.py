...
def test_experiments_marian_1_10(wandb_mock, getargs_mock, caplog, samples_dir, tmp_dir):
    ...
    assert set([(level, message) for _module, level, message in caplog.record_tuples]) == set(
        [
            ...
            (logging.INFO, "Creating missing run backward-1 with associated metrics"),
            (logging.INFO, "Creating missing run quantized-1 with associated metrics"),
            (logging.INFO, "Creating missing run student-finetuned-1 with associated metrics"),
            ...
            (logging.INFO, "Creating missing run teacher-ensemble-1 with associated metrics"),
        ]
    )
